---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Amp, Cursor Agent CLI, MastraCode, Antigravity CLI, and Grok CLI.
---
...
herdr integration uninstall amp
...
| Session identity | Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Qoder CLI, Qwen Code, Letta Code, Amp, Cursor Agent CLI, Hermes Agent, Antigravity CLI, Grok CLI | The integration reports native session references for restore. State still comes from Herdr's screen manifest detection. |
...
Amp integration installation is experimental and CLI-only. It is not listed in the Settings integration screen or the JSON integration API. `herdr integration status` reports it as `amp (experimental)`. Agent detection and `herdr agent start --kind amp` are supported independently of this installation restriction.
...
The plugin reports the active Amp thread ID when a session starts inside a Herdr pane. It reports only session identity: Amp state continues to come from Herdr's screen manifest detection.

Herdr uses `$XDG_CONFIG_HOME/amp` when `XDG_CONFIG_HOME` is set, otherwise `~/.config/amp`. The Amp config directory must already exist. Install writes `plugins/herdr-agent-session.ts`; uninstall removes only that plugin file.

Herdr resumes stored Amp threads with `amp threads continue <id>`.
