...
pub(crate) use actions::{
    install_experimental_amp, install_experimental_letta, install_target,
    uninstall_experimental_amp, uninstall_experimental_letta, uninstall_target,
};
#[cfg(test)]
...
pub(crate) use registry::{
    experimental_amp_integration_status, experimental_letta_integration_status,
...
pub(crate) const EXPERIMENTAL_INTEGRATION_TARGET_LABELS: &[&str] = &["letta", "amp"];

const AMP_PLUGIN_INSTALL_NAME: &str = "herdr-agent-session.ts";
const AMP_PLUGIN_ASSET: &str = include_str!("assets/amp/herdr-agent-session.ts");
